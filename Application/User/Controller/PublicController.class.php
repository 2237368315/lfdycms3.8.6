...
class PublicController extends Controller {
	 ...
	 protected function _initialize(){
        ...
        $mobile=is_mobile()?'wap':'web';
        C('CACHE_PATH',RUNTIME_PATH."/Cache/".MODULE_NAME."/".ucfirst($mobile)."/");
        $this->tplpath=$this->webpath.C("TPL_PATH").$mobile.'/'.C("DEFAULT_".strtoupper($mobile)."_TPl");
        C('VIEW_PATH',APP_PATH.MODULE_NAME.'/'.C('DEFAULT_V_LAYER').'/'.$mobile.'/');
        C('TMPL_ACTION_ERROR','Public/dispatch_jump');
        C('TMPL_ACTION_SUCCESS','Public/dispatch_jump');
        if(!C('WEB_SITE_CLOSE')){
	...
	public function reg(){
		...
		if(IS_POST){
			...
			$uid = D('Public')->reg();
			if(is_numeric($uid)){
                $this->success('用户注册成功！',U('User/Public/login'));
            } else {
